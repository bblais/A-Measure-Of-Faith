...
def task_md_html_individual():
    ...
    for name in files:
        ...
        s= 'pandoc  -M date="`date`" --filter eqn2png.py --citeproc  --filter replace_metavars.py -s --toc --toc-depth=2 --top-level-division=chapter --embed-resources --standalone -o "%s" "%s" config/config.yaml' % (htmlfile,mdfile)
...
def task_tex_pdf_6x9():
    return {
        ...
        'actions': ['pdflatex -halt-on-error -output-directory build build/measure_of_faith_6x9.tex', 
                    ...
                    'mv -f build/measure_of_faith_6x9_embeded.pdf build/measure_of_faith_6x9.pdf',
                    'cp build/measure_of_faith_6x9.pdf "./A Measure of Faith - Brian Blais 6x9.pdf"'],
        'clean':[clean_targets,'rm -f build/*.log build/*.toc build/*.aux build/*.out build/*.bbl build/*.blg build/*.synctex.gz'],
        'verbosity': 2,
    }
